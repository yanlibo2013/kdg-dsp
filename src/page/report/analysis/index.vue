        ...
        小时</el-checkbox>
                ...
                checkedSpace:false, -->
                    <div class="customer-column clear">
                        ...
                        <!-- <el-checkbox-group v-model="checkedCustom" @change="handleCheckedResourceChange"> -->
                            <!-- <el-checkbox-group  > -->
                        <table min-width="500px" class="customer-msg resource-wrap" border="0" cellpadding="0" cellspacing="0">
                            <tbody>
                                <tr class="space-under">
                                    <td>
                                    <template>
                                        <!-- <el-checkbox label="客户" key="客户" v-model="checkedCustom" @change="changeAd(checkedCustom)">
                                            客户：</el-checkbox> -->
                                            <el-checkbox label="客户" 
                                            style="width:78px"
                                            :disabled="checkedCustomId"
                                            key="客户" 
                                            v-model="checkedCustomId"
                                            @change="changeCustom(checkedCustomId)">
                                            客户：</el-checkbox>
                                    </template>
                                    </td>
                                    <td>
                                        <el-select class="ajust-full-width2"
                                         v-model="valueAdvertiser"  
                                         placeholder="请选择" 
                                         multiple
                                         collapse-tags
                                         @change="getAdvertiser(valueAdvertiser)" 
                                         filterable>
                                            <el-option 
                        ...
                        <el-option v-for="item in planOptions" 
                         ...
                         :value="item.value">
                                ...
                                <tr class="space-under">
                                    <td>
<template>
    <!-- <el-checkbox label="创意" key="创意" v-model="checkedCreative">
        创意：</el-checkbox> -->
         <el-checkbox label="创意" key="创意"  style="width:78px" v-model="checkedCreative" @change="changeCreative(checkedCreative)">
        创意：</el-checkbox>
</template>
                                    </td>
                                    <td>
        ...
        尺寸：</el-checkbox>
                                    ...
                                    <td>
                                         ...
                                         @change="selectSize(size)" 
                                          filterable class="ajust-full-width2 chicun" >
                                            <el-option
                            ...
                            <tr class="space-under">
                                    ...
                                    <td>
                                        <el-select 
                                             class="ajust-full-width2 chicun"  
                                             multiple
                                             collapse-tags
                                             v-model="service" 
                                             placeholder="请选择"
                                             @change="showService(service)" 
                                         >
                                ...
                                <tr class="space-under">
                                    <td>
<template>
    <el-checkbox label="展现类型" key="展现类型" v-model="checkedViewType" @change="changeViewType(checkedViewType)">
        展现类型:</el-checkbox>
</template>
                                    </td>
                                    <td>
                                   ...
                                   <tr class="space-under">
                                    <td>
                                    <template>
                                        <el-checkbox label="平台" key="平台" 

                                        v-model="checkedPlatType"
                                        @change="changePlatTypeCheck(checkedPlatType)">
                                            平台：</el-checkbox>
                                    </template>
                                    </td>
                                    <td>

                                        <el-select class="ajust-full-width2 chicun" 
                                        v-model="plat" 
                                        placeholder="请选择" 
                                        multiple
                                        collapse-tags
                                        @change="changePlatType(plat)"
                                        >
        ...
        广告位：</el-checkbox>
                ...
                <div class="display-dimension border-bottom-2">
                    ...
                    <div class="show-bd">
                        <div class="checkbox-group-item clear">
                            <label for="" class="show-title left" v-show="setPermission(51) || setPermission(37)">金额：</label>
                             <el-checkbox label="消耗" key="消耗" v-show="setPermission(51)">消耗</el-checkbox>
                             <el-checkbox label="成本" key="成本" v-show="setPermission(37)">成本</el-checkbox>
                             <el-checkbox label="平均成本" key="平均成本" v-show="setPermission(37)">平均成本</el-checkbox>
                        </div>
                        <!-- <div class="checkbox-group-item clear">
                            <label for="" class="show-title left">数据：</label>
                        ...
                        <div class="checkbox-group-item clear">
                            ...
                            <!-- <el-checkbox label="千次曝光成本" key="千次曝光成本">千次曝光成本（CPM）</el-checkbox> -->
                        </div>
                        <div class="checkbox-group-item clear">
                            <label for="" class="show-title left">效果：</label>
                            <el-checkbox label="点击数" key="点击">点击数</el-checkbox>
                            <el-checkbox label="点击率" key="点击率">点击率</el-checkbox>
                            <!-- <el-checkbox label="点击单价" key="点击单价">点击单价（CPC）</el-checkbox> -->
                        </div>
                ...
                <div class="export-report border-bottom-2">
                    <div class="export-hd">
                        ...
                        <el-select class="ajust-full-width2" v-model="paramsObj.ext" placeholder="请选择">
                    ...
                    if (res.data.code == 0) {
                        if(res.data.data.data[0]){
                            ...
                            for(let key in this.currentItem){
                                ...
                                label!=""&&this.cols.push({prop: key, label:label,index:index});
                            ...
                            if(JSON.parse(data.customer).uid && JSON.parse(data.customer).uid.ch){
                                 this.cols.push({
                                  label:"客户"
                                 })
                            }
                             if(JSON.parse(data.customer).campaign_id && JSON.parse(data.customer).campaign_id.ch){
                                 this.cols.push({
                                  label:"计划"
                                 })
                            }
                             if(JSON.parse(data.customer).creative_id && JSON.parse(data.customer).creative_id.ch){
                                 this.cols.push({
                                  label:"创意"
                                 })
                            }
                             ...
                             if(JSON.parse(data.customer).deviceType && JSON.parse(data.customer).deviceType.ch){
                                 this.cols.push({
                                  label:"设备类型"
                                 })
                            }
                             if(JSON.parse(data.customer).type_id && JSON.parse(data.customer).type_id.ch){
                                 this.cols.push({
                                  label:"展现类型"
                                 })
                            }
                             ...
                             if(JSON.parse(data.customer).media_id && JSON.parse(data.customer).media_id.ch){
                                 this.cols.push({
                                  label:"媒体"
                                 })
                            }
                             if(JSON.parse(data.customer).plat_id && JSON.parse(data.customer).plat_id.ch){
                                 this.cols.push({
                                  label:"平台"
                                 })
                            }
                             if(JSON.parse(data.customer).adposition_id && JSON.parse(data.customer).adposition_id.ch){
                                 this.cols.push({
                                  label:"广告位"
                                 })
                            }
                            if(data.present.length>0){
                               for(let k=0;k<data.present.length;k++){
                                    this.cols.push({
                                        label:data.present[k]
                                 })
                               }
                            }
                           this.cols=_.orderBy(this.cols, ['index'], ['asc'])
                        }
...
<style lang="scss">
    ...
    .base-data-report {
        ...
        .effectform .ajust-full-width2{
            .el-select__tags{
                //max-width: 240px !important;
            }
        }

        .effectform .ajust-full-width2  .el-select__tags>span>.el-tag--info:nth-child(1)>span{
                //   color: red;
                //   max-width: 192px;
                //   display: inline-block;
          }

        .effectform .ajust-full-width2  .el-select__tags>span :nth-child(1) .el-select__tags-text{
            //     max-width: 130px;
            //     overflow: hidden;
