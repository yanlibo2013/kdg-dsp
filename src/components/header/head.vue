...
<template>
    <el-header>
        <div class="path left">
            <template v-for="item in currentbreadcrumb">
                <template v-if="!item.last">
                    ...
                    <i class="path-split" :key="item.title">&gt;</i>
                </template>
                <template v-else>
        ...
        <div class="hd-bd">
            <!-- <el-select v-model="admodel" placeholder="默认客户" @change="adchange" filterable>
                <el-option v-for="item in advertiser" :key="item.id" :label="item.com_name" :value="item.id">
                </el-option>
            </el-select> -->
            <custome v-show="setIshowCustome"></custome>
...
export default {
    ...
    computed: {
        ...
        setIshowCustome () {
            ...
            if (this.$route.path.replace("/", "") == "home" || this.$route.path.replace("/", "") == "list" || this.$route.path.replace("/", "").indexOf("plantable/") == 0 || this.$route.path.replace("/", "").indexOf("relation/") == 0) {
        ...
        serBreadCrumb () {
            ...
            console.log(this.$store.state.currentbreadcrumb);
            let temp = '<div class="path left">' +
                ' <template v-for="' + item in this.$store.state.currentbreadcrumb + '">'
            '<template v-if="' + !item.last + '">'
            '<router-link :to="' + item.path + '" :key="' + item.id + '">{{' + item.title + '}}</router-link>'
            '<i class="path-split" :key="' + item.title + '">&gt;</i>'
            '</template>'
...
<style lang="scss">
    ...
    .el-header {
     ...
     .el-header .path a, .el-header .hd-title{
